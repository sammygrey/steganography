...
def decode_image(path_to_png):
    ...
    decoded_image.save(f"decoded_{Path(path_to_png).stem}.png")
    decoded_image.show()
...
def encode_image(path_to_png, text_to_write):
    ...
    """
    unencoded_image = Image.open(path_to_png)
    ...
    for x in range(x_size):
        for y in range(y_size):
            if txt.getpixel((x,y)) == 255: #if pixel in text image is white
                p = red.getpixel((x, y))
                b = format(p, "b")[:-1] + "1"
                d = int(b,2)
                red.putpixel((x, y), d)
    ...
    encoded_image.save(f"encoded_{Path(path_to_png).stem}.png") 
...
def write_text(text_to_write, size):
    ...
    txt.text((10,10), text_to_write, fill=(11111111))
    return im